...
var Preview = Widget.extend({
    ...
    init: function (parent, options) {
        this.title = options.title.slice(0,70) //googleSERPTitleSize);
            if (this.title != options.title) {
                this.title = this.title.concat("...");
            }
        if (options.url.indexOf("https") != 0) {
            this.url = options.url.slice(7,78) //widget.httpURLSize); // "http://" is 7
        } else {
            this.url = options.url.slice(0,90) //widget.httpsURLSize);
        }
...
var Configurator = Dialog.extend({
    ...
    maxTitleSize: 110, // limit for site name SERP is 600 pixel (~70 char.), but Title size is not limited
    maxDescriptionSize: 300,  // Google limit in november 2017
    googleSERPTitleSize: 70, // Bing is 40
